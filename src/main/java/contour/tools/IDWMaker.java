...
public class IDWMaker {
    public static void testCountry(double[][] dataArr, double[][] bounds,int zoom, String outPutPath) {
        ...
        List<Tuple5<Double, Double, Integer, Integer, Integer>> colors = getColors(filePath + "color.csv");
        IDWImage idwImage = new IDWImage(dataArr, colors, bounds, outPutPath, filePath, crsParams);
        idwImage.draw();
